...
public class CheckoutDialog extends JDialog {
		...
		btnCheckout.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(UserManager.isBan()) {
					AlertDialog.printMsg("도서 대출 / 예약이 불가능한 상태입니다.");
					return;
				}	
				String userId = UserManager.getUserId();
				String state = OracleBookQuery.BOOK_STATE_OUT;
				LocalDateTime outTime = LocalDateTime.now();
				LocalDateTime returnTime = outTime.plusDays(MAX_CHECK_OUT_TIME);
	...
	private void printBookInfo(BookVO vo) {
		...
		if(vo.getState().equals(OracleBookQuery.BOOK_STATE_OUT) || 
				vo.getState().equals(OracleBookQuery.BOOK_STATE_RSV)) {
			returnDate = dao.selectCheckinDate(vo.getBookId());
			contents =contents.concat("반납 예정일 : " + returnDate.toLocalDate().toString());
		}
